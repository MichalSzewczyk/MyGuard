...
public class CrimesRestApiClientImpl implements CrimesRestApiClient {
    ...
    private static final String STREET_LEVEL_CRIMES_AT_LOCATION = "stops-street?lat=%s&lng=%s";
    ...
    @Override
    public Crime[] getCrimesForLocation(double lat, double lng) {
        return restService.parseJsonData(restService.getJsonForRequest(
                String.format(STREET_LEVEL_CRIMES_AT_LOCATION, String.valueOf(lat), String.valueOf(lng))));
    }
}
